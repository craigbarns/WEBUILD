...
Le propriétaire a confirmé WEBUILD SAS, SIREN `901806000`, comme société qui signe les devis et facture les travaux, ainsi que le siège `41 rue Fongate, 13006 Marseille`, le téléphone `07 66 01 88 26` et l’email `contact@webuildmarseille.fr`. `verifiedContact`, `verifiedAddress` et `verifiedLegalIdentity` sont renseignés dans `site.config.json`, avec textes visibles et tests cohérents. Les profils externes restent à vérifier avant de remplir `verifiedSameAs`. L’adresse du siège ne prouve pas un accueil de clientèle : vérifier ce point avant l’affichage d’adresse sur GBP.
